 ...
 *       against the bytes left before anything is built from it.
...
export type CollectionDbWarning = {
  ...
  /**
   * The hash's place in that collection's list in the file, or null for a warning about a name.
   * It's the index into `hashes` unless a null hash was dropped before it.
   */
  hash: number | null;
};

/** A read database, plus anything odd the reader found in it. */
export type CollectionDbRead = CollectionDb & { warnings: readonly CollectionDbWarning[] };

export type ReadCollectionDbOptions = {
  /** Largest input accepted, in bytes. Default MAX_COLLECTION_DB_BYTES (64 MiB). */
 ...
 * @returns {CollectionDbRead} the version, the collections in file order, and warnings
 * @throws {CollectionDbError} for a file it can't read (see CollectionDbErrorCode)
...
export const readCollectionDb = (
  ...
  const warn = (
    ...
    warnings.push({ code, offset: at, collection, hash });
  };
  ...
  const readString = (collection: number, hash: number | null): string | null => {
    ...
    if (marker !== STRING_MARKER) {
      ...
      warn("unknown_marker", markerAt, collection, hash);
    }
    ...
    try {
      ...
      warn("invalid_utf8", start, collection, hash);
  ...
  for (let c = 0; c < count; c++) {
    ...
    for (let h = 0; h < hashCount; h++) {
      const hashAt = offset;
      const hash = readString(c, h);
      if (hash === null) {
        warn("null_hash", hashAt, c, h);
        continue;
      }
      if (!LOWERCASE_MD5.test(hash)) {
        warn(HEX_32.test(hash) ? "uppercase_hash" : "malformed_hash", hashAt, c, h);
      }
      if (seen.has(hash)) warn("duplicate_hash", hashAt, c, h);
      else seen.add(hash);
      hashes.push(hash);
    }
  ...
  return { version, collections, warnings };
};
