 ...
 *       typechecks a consumer strictly (no skipLibCheck, so broken .d.ts can't hide as `any`; the
 *       DOM lib and no Node types, as in a browser) and runs it. Proves the zod peer range's
 *       floor and that every entry point resolves from the packed tarball. Usage: node
...
const { hashes } = collectionHashesFor([meta]);
const collectionDb: CollectionDb = addToCollection(createCollectionDb(), "Tourney practice", hashes).db;
